...
bool produceOne(const std::string& host, uint16_t port, const std::string& topic,
    ...
    if (!s.connectTo(host, port)) {
        std::cerr << "Failed to connect to broker at " << host << ":" << port << "\n";
        return false;
    }
    ...
    if (!writeFrame(s, static_cast<uint8_t>(RequestType::Produce), encodeProduceRequest(req))) {
        std::cerr << "Failed to send produce request\n";
        return false;
    }
    ...
    if (!readFrame(s, status, respPayload)) {
        std::cerr << "Failed to read broker response (connection closed?)\n";
        return false;
    }
    if (static_cast<StatusCode>(status) != StatusCode::Ok) {
        std::cerr << "Broker returned error status " << static_cast<int>(status) << "\n";
        return false;
    }

    ProduceResponse resp;
    if (!decodeProduceResponse(respPayload.data(), respPayload.size(), resp)) {
        std::cerr << "Failed to decode produce response\n";
        return false;
    }

    std::cout << "Message sent -> topic \"" << topic << "\", partition " << resp.partition
               << ", offset " << resp.offset;
    if (key.has_value()) {
        std::cout << " (key: \"" << *key << "\")";
    } else {
        std::cout << " (no key - broker chose the partition by round robin)";
    }
    std::cout << "\n";
    return true;
}
...
int main(int argc, char** argv) {
    ...
    if (positional.empty()) {
        std::cerr << "Usage: minikafka-produce [--host=H] [--port=P] <topic> [key] [value]\n"
                     "  <topic> only          - reads lines from stdin as unkeyed message values\n"
                     "  <topic> <value>       - produce one unkeyed message\n"
                     "  <topic> <key> <value> - produce one keyed message\n";
        return 1;
