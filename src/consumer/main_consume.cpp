...
// failure (unexpected).
enum class MembershipResult { Ok, OwnedByAnother, Failed };
...
MembershipResult sendMembershipRequest(RequestType type, const std::string& host, uint16_t port,
    ...
    Socket s;
    if (!s.connectTo(host, port)) return MembershipResult::Failed;
    ...
    if (!readFrame(s, status, payload)) return MembershipResult::Failed;
...
bool fetchCommittedOffset(const std::string& host, uint16_t port, const std::string& group,
    ...
    Socket s;
    if (!s.connectTo(host, port)) return false;
    ...
    uint8_t status;
    std::vector<uint8_t> payload;
    if (!readFrame(s, status, payload)) return false;
    if (static_cast<StatusCode>(status) != StatusCode::Ok) {
        std::cerr << "FetchOffset error status " << static_cast<int>(status) << "\n";
        return false;
    }

    FetchOffsetResponse resp;
    if (!decodeFetchOffsetResponse(payload.data(), payload.size(), resp)) return false;
...
bool fetchRecords(const std::string& host, uint16_t port, const std::string& topic,
    ...
    if (!readFrame(s, status, payload)) return false;
    if (static_cast<StatusCode>(status) != StatusCode::Ok) {
        std::cerr << "Fetch error status " << static_cast<int>(status) << "\n";
        return false;
    }
...
int main(int argc, char** argv) {
    ...
    if (positional.size() != 3) {
        std::cerr << "Usage: minikafka-consume [--host=H] [--port=P] [--follow] "
                     "[--consumer-id=ID] <topic> <partition> <group>\n";
        return 1;
    }
    ...
    if (joinResult == MembershipResult::OwnedByAnother) {
        std::cerr << "Cannot start: group \"" << group << "\" is already actively consuming topic \""
                   << topic << "\", partition " << partition << " (another consumer holds it).\n"
                   << "Use a different group name, or wait for that consumer to finish/time out.\n";
        return 1;
    }
    if (joinResult == MembershipResult::Failed) {
        std::cerr << "Failed to join group - is the broker running at " << host << ":" << port
                   << "?\n";
        return 1;
    }
    ...
    if (!fetchCommittedOffset(host, port, group, topic, partition, consumerId, offset)) {
        std::cerr << "Failed to fetch committed offset from broker at " << host << ":" << port
                   << "\n";
        exitCode = 1;
    } else {
        if (offset < 0) {
            offset = 0;
            std::cout << "No previous progress found for group \"" << group << "\" on topic \""
                      << topic << "\", partition " << partition
                      << " - starting from the beginning.\n";
        } else {
            std::cout << "Resuming group \"" << group << "\" on topic \"" << topic
                      << "\", partition " << partition << " from offset " << offset
        ...
        do {
            ...
            if (!fetchRecords(host, port, topic, partition, offset, 100, resp)) {
                std::cerr << "Fetch failed - is the broker running at " << host << ":" << port
                           << "?\n";
                exitCode = 1;
                break;
            }
            ...
            if (resp.records.empty()) {
                ...
                std::cout << "Received " << resp.records.size() << " message(s):\n";
            ...
            if (!resp.records.empty()) {
                ...
                if (!commitOffset(host, port, group, topic, partition, offset, consumerId)) {
                    std::cerr << "Warning: failed to save progress at offset " << offset << "\n";
                }
            } else if (follow) {
